...
2. **Backend config files are gitignored**: `backend/.env` and `backend/config/services.yaml` must exist for the app to work. These are not committed to the repo. The Docker entrypoint expects them to be present.
